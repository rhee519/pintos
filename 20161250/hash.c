...
/*
    hash_elem 포인터를 포함하는 hash_item 포인터를 return
*/
struct hash_item *get_hash_item(const struct hash_elem *e)
{
    return hash_entry(e, struct hash_item, elem);
}

/*
    delete hash 호출 시 메모리 해제를 위한 destructor
*/
void hash_destructor(struct hash_elem *e, void *aux)
{
    struct hash_item *it = get_hash_item(e);
    if (it)
        free(it);
}
/*
    hash 초기화 시 주입하는 hash function으로, hash_int() 알고리즘이 적용됨
*/
unsigned hash_int_func(const struct hash_elem *e, void *aux)
{
    struct list_item *list_item = get_list_item(&e->list_elem);
    return hash_int(list_item->data);
}

/*
    hash_elem 두 개의 int 값을 비교
*/
bool hash_less_int_func(const struct hash_elem *a,
                        const struct hash_elem *b,
                        void *aux)
{
    struct hash_item *a_item = get_hash_item(a);
    struct hash_item *b_item = get_hash_item(b);
    return a_item->data < b_item->data;
}

/*
    입력받은 int num의 bit를 모두 flip한 결과를 unsigned로 반환
*/
