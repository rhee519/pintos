...
/* [0-2] Jiho Rhee */
/*
  bitmap의 각 bit를 MSB부터 이진법으로 print
*/
void bitmap_dumpdata(const struct bitmap *b)
{
  if (b == NULL || b->bit_cnt == 0)
    return;
  // printf("%zu\n", b->bit_cnt);

  for (size_t i = 0; i < b->bit_cnt; i++)
  {
    elem_type mask = bit_mask(i);
    size_t idx = elem_idx(i);
    bool bit = mask & b->bits[idx];
    printf("%d", bit ? 1 : 0);
  }
  printf("\n");
}
...
/*
  기존 bitmap의 뒷부분에 size개의 0 bit를 expand(pad)
  성공 시 bitmap pointer를, 실패 시 NULL을 반환
*/
struct bitmap *bitmap_expand(struct bitmap *b, size_t size)
{
  size_t old_size = bitmap_size(b);
  struct bitmap *new = bitmap_create(old_size + size);
